(() => {
  ...
  const loadBook = (key, done) => {
    ...
    script.src = `vocabulary-data-${key}.js?v=20260822-1749`;
  ...
  function renderStudy() {
    ...
    const question = state.mode === 'spelling' ? `<div class="meaning spelling-clue">${escapeHtml(current.meaning)}</div><input class="spelling-input" id="spelling" autocomplete="off" autocapitalize="none" placeholder="输入英文单词"><button class="submit" data-action="spell">检查拼写</button><div id="feedback"></div>` : `<div class="word">${escapeHtml(current.word)}</div><div class="phonetic">${escapeHtml(current.phonetic)}</div>`;
    const activity = state.mode === 'choice' ? `<div class="choices">${state.choices.map(item=>`<button data-action="choice" data-id="${item.id}">${escapeHtml(item.meaning)}</button>`).join('')}</div><div id="feedback"></div>` : state.mode === 'card' ? '<button class="reveal" data-action="reveal">显示释义</button><div id="answer"></div>' : '';
  ...
  const answerHtml = () => `<div class="answer"><div class="meaning">${escapeHtml(state.current.meaning)}</div><div class="example">${escapeHtml(state.current.example)}</div></div>`;
