(() => {
  ...
  function renderList(filter,limit=200){state.view='list';state.filter=filter;state.listLimit=limit;const today=day(),query=state.listQuery.trim().toLowerCase();const rows=book().words.map(word=>({...word,state:wordState(word)})).filter(x=>filter==='favorite'?x.state&&x.state.favorite:filter==='due'?x.state&&x.state.due<=today:filter==='mastered'?x.state&&x.state.mastered:true).filter(x=>!query||x.word.toLowerCase().includes(query)||x.meaning.toLowerCase().includes(query));const shown=rows.slice(0,limit);root.innerHTML=`${brand}<button class="topline" data-action="book-back">‹ 返回词书</button><section class="hero"><h1>单词<span>列表。</span></h1></section><div class="word-search"><input id="word-search" value="${state.listQuery.replace(/"/g,'&quot;')}" placeholder="搜索单词或中文释义"><button data-action="word-search">搜索</button></div><div class="list-count">共 ${rows.length} 词 · 已显示 ${shown.length}</div><div class="filters">${[['all','全部'],['due','待复习'],['favorite','收藏'],['mastered','已掌握']].map(x=>`<button class="${filter===x[0]?'active':''}" data-action="list" data-filter="${x[0]}">${x[1]}</button>`).join('')}</div>${shown.length?shown.map(x=>`<div class="word-row"><div><b>${x.word}</b><small>${x.phonetic} · ${x.meaning}</small>${x.bookPage?`<small>词汇书第 ${x.bookPage} 页 · ${x.levelName||''}</small>`:''}</div><span>${!x.state?'未学习':x.state.mastered?'已掌握':x.state.due<=today?'待复习':'学习中'}</span></div>`).join(''):'<div class="empty">这里还没有单词</div>'}${shown.length<rows.length?'<button class="load-more" data-action="list-more">继续显示200词</button>':''}`}
