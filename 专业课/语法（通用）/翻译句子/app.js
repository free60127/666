...
(() => {
  ...
  root.innerHTML = sections.map(section => {
    ...
    return `<section class="exercise" id="ex-${section.title}"><div class="exercise-head"><span class="badge">${section.title}</span><div><h2>EXERCISE ${section.title}</h2><small>${section.items.length} 题</small></div><button class="print-button" type="button" data-print="${section.title}">导出 A4 PDF</button></div>${note}${items}</section>`;
  }).join('');
