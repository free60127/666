...
(() => {
  ...
  nav.innerHTML = sections.map(section => `<a href="#ex-${section.title}">${section.title}</a>`).join('');
  root.innerHTML = sections.map(section => {
    ...
    return `<section class="exercise" id="ex-${section.title}"><div class="exercise-head"><span class="badge">${section.title}</span><div><h2>EXERCISE ${section.title}</h2><small>${section.items.length} 题</small></div></div>${note}${items}</section>`;
  }).join('');
  root.addEventListener('click', event => { if(!event.target.matches('.answer-toggle')) return; const answer = event.target.nextElementSibling; answer.classList.toggle('show'); event.target.textContent = answer.classList.contains('show') ? '收起参考译文' : '查看参考译文'; });
})()
