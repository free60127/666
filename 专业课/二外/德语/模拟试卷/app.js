(() => {
  ...
  function renderHome() {
    app.innerHTML = `<button class="back hub-back" data-action="hub">‹ 返回德语</button><header class="brand"><div class="mark">卷</div><div><strong>${escape(pageTitle)}</strong><small>MUSTERPRÜFUNG / 2026</small></div></header>
      <section class="hero"><small>DEUTSCH · MOCK EXAM</small><h1>选择你的<span>模拟试卷。</span></h1><p>参考《新编大学德语》期末试卷题型，每三个单元一套，在线作答自动判题。</p></section>
      <section class="bank-list">${data.map(book => `<button class="bank-card" data-action="book" data-book="${book.key}"><div><b>${escape(book.name)}</b><em>进入</em></div><h2>${escape(book.name)}</h2><p>${totalQ(book)} 题 · ${book.units.length} 个题型</p><small>展开题型即可作答，选择/填空自动判题</small></button>`).join('')}</section>`;
  }
