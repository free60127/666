(() => {
  ...
  function renderHome() {
    app.innerHTML = `<button class="back hub-back" data-action="hub">‹ 返回通用课程</button><header class="brand"><div class="mark">R</div><div><strong>${escape(pageTitle)}</strong><small>CONTENT LIBRARY / 2026</small></div></header>
  ...
  function renderBook(key) {
    ...
    app.innerHTML = `<button class="back" data-action="home">‹ 返回教材列表</button><section class="hero compact"><small>${escape(book.name)}</small><div class="stats"><span><b>${book.units.length}</b>单元</span><span><b>${totalQ(book)}</b>题目</span></div></section>
      ${book.units.length ? `<div class="actions"><button data-action="export">导出全部题目 A4 PDF</button><button data-action="export-opened">导出已展开单元 PDF</button></div>
      <p class="notice">按题型展开单元；选择题点击选项判题，其余题型展开参考答案。</p>` : '<p class="notice">本模块资料整理中，题目会陆续发布在这里。</p>'}
