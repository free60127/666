(() => {
  ...
  document.addEventListener('click', event => {
    ...
    setTimeout(() => {
      // 思政/计算机判题后整卡重渲染（页面 render()），事件目标已脱离 DOM——
      // 先从点击坐标反查新卡，拿不到再退回到旧卡上的 key 等本引擎 enhance 完成后补记。
      let current = card;
      try {
      ...
      if (!current.dataset.unifiedReady) {
        ...
        setTimeout(() => {
          ...
          if (!re) return;
          const result = resultFromCard(re);
          record(re, result === null ? undefined : result);
          prepareCard(re);
        }, 150);
        return;
      }
