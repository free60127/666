(() => {
  ...
  const copyCloudCode = () => callCloudPanel('copyCloudCode');
  const doCloudBackup = () => callCloudPanel('doCloudBackup');
  const doCloudRestore = input => callCloudPanel('doCloudRestore', input);
...
<h2 class="section-title">最近学习</h2>${recent.length ? `<div class="items">${recent.map(item => itemCard(item,'progress')).join('')}</div>` : empty('还没有学习记录','在网页题库中答题或展开答案后，这里会自动记录。')}<p class="note">网页端记录保存在当前浏览器中；更换设备或清理浏览器数据后不会自动同步。导出备份会包含网页题库、背单词、思政/计算机答题状态与查词收藏。</p>`;
  ...
  document.addEventListener('click', event => {
    ...
    if (action === 'cloud-restore') {
      Promise.resolve(showCloudPanel(true)).then(() => {
        refreshCloudCodeBox();
