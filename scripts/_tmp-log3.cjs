...
async function main() {
  ...
  const idx = lines.findIndex(l => l.includes('验证结果') || l.includes('结果：') || l.includes('wrangler dev') || l.includes('Error:') || l.includes('FAIL'));
  console.log(lines.slice(Math.max(0, idx - 10), idx + 60).join('\n'));
}
