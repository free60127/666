// 基英/泛读（内容页）：普通单选 / CF 六选项多选 / A or D / 多空填空 / 翻译自评
...
async function locate(page, predBody) {
  await page.goto('/专业课/英语系/基英系列/index.html');
...
test('多空填空（逗号答案必须完整）：只填部分判错，完整填写判对', async ({ page }) => {
  const loc = await locate(page, `(q, u) => q.type === 'text' && ['wordFill', 'vocabulary', 'word', 'fill'].includes(u.kind) && /,/.test(String(q.answer || '')) && String(q.answer).split(',').length >= 3`);
  expect(loc).not.toBeNull();
  const card = await openCard(page, loc);
  // 只填第一个词 → 必须判错
...
test('分号多合法答案：填任一即判对', async ({ page }) => {
  const loc = await locate(page, `(q, u) => q.type === 'text' && ['wordFill', 'vocabulary', 'word', 'fill'].includes(u.kind) && /;/.test(String(q.answer || ''))`);
  expect(loc).not.toBeNull();
  const card = await openCard(page, loc);
